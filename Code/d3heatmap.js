###
# Modified from the original d3heatmap package
#
###
    ...
    this.loadImage(x.image, function(imgData, w, h) {
      ...
      if (window.Shiny) {
        ...
        hm.on('hover', function(e) {
          Shiny.onInputChange('heatmap_hover', !e.data ? e.data : {
            label: e.data.label,
        ...
        hm.on('click', function(e) {
          Shiny.onInputChange('heatmap_click', !e.data ? e.data : {
              label: e.data.label,
	      row: x.matrix.rows[e.data.row],
	      col: x.matrix.rows[e.data.row]
          });
        });
